  ...
  const processFiles = async () => {
    ...
    for (const uploadFile of files) {
      ...
      try {
        ...
        setFiles((prev) => prev.map((f) => (f.id === uploadFile.id ? { ...f, status: "uploading", progress: 25 } : f)))
        ...
        setFiles((prev) => prev.map((f) => (f.id === uploadFile.id ? { ...f, status: "processing", progress: 50 } : f)))

        // Process with AI backend
        const aiResult = await aiClient.analyzeDocument(uploadFile.file, documentType)

        // Update the document with AI results
        await fetch(`/api/documents/${uploadResult.documentId}/process`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({
            ocrText: aiResult.ocr.text,
            entities: aiResult.entities,
            confidence: aiResult.ocr.confidence,
          }),
        })
          ...
          <CardContent className="p-6">
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Button onClick={processFiles} disabled={isProcessing || !claimId || !documentType} className="ml-auto">
                  {isProcessing ? (
                    <>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {uploadFile.result && (
                        <div className="mt-2 p-2 bg-muted rounded text-xs">
                          <p>OCR Confidence: {(uploadFile.result.ocr.confidence * 100).toFixed(1)}%</p>
                          <p>Entities Found: {Object.keys(uploadFile.result.entities).length}</p>
                        </div>
                      )}
                    </div>
